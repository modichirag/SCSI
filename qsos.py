...
parser.add_argument("--max_spectra", type=int, default=1000, help="max QSO spectra to download (small default ~15min; paper used ~40000)")
parser.add_argument("--z_min", type=float, default=2.75, help="QSO redshift lower bound")
parser.add_argument("--z_max", type=float, default=3.25, help="QSO redshift upper bound")
...
# Load and construct dataset via the QSO registry (downloads on first run)
qso_data, _, _ = get_dataset(
    'qso', args.data_root,
    max_spectra=args.max_spectra,
    z_range=(args.z_min, args.z_max),
)
full_flux = qso_data.flux      # (N, 2999)
full_loglam = qso_data.loglam  # (2999,)
print("Loaded spectra of shape : ", full_flux.shape)
# Crop + stride-subsample from native length down to ~D pixels
subs = full_flux.shape[-1] // D
remainder = full_flux.shape[-1] % D
i0, i1 = remainder // 2, -(remainder // 2)
spectra = full_flux[:, i0:i1:subs]
wavelength = torch.from_numpy(10.0 ** full_loglam[i0:i1:subs])
...
if args.combinedsde:
    interpolant = SCSInterpolantCombined(fwd_func, use_latents=use_latents, \
